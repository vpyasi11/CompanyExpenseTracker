...
export default function CashFlowLayout() {
  ...
  React.useEffect(() => {
    const fetchSheet = async () => {
      try {
        ...
        const response = await axios.get(`${baseUrl}/cashflowstatement/${id}`);
        console.log("response based oncompnyId :", response.data);
        setCashFlowSheet(response.data.cashFlowSheets);
  ...
  return (
    <div style={divStyle}>
      <Grid item xs={6} >
        <Button style={{ border: "1px solid #B7B7B7", marginTop: "4px" }} onClick={() => navigate('/cashflow/analysis')}>View Analysis</Button>
      </Grid>
      ...
      <TableContainer component={Paper} style={{ marginTop: '8px', width: "1200px" }}>
        <Table sx={{ minWidth: 500 }} aria-label="customized table">
          <TableHead >
            <StyledTableRow>
              ...
              {cashFlowSheet.slice(page * 2 - 2, page * 2).map((item, index) => {
          ...
          <TableBody>
            {Array.from(new Set(cashFlowSheet.flatMap(sheet => Object.keys(sheet)))).map((key) => {
              if (key.toLowerCase() !== '_id') { // Exclude 'ID' key
                let formattedKey = formatString(key);
                return (
                  <StyledTableRow key={key}>
                    <StyledTableCell>{formattedKey}</StyledTableCell>
                    {cashFlowSheet.slice(page * 2 - 2, page * 2).map((sheet, index) => {
        ...
        <Stack spacing={2}>
          ...
          <Pagination count={cashFlowSheet.length} page={page} size="large" onChange={handleChange} color="primary" />
        </Stack>
      </TableContainer>
